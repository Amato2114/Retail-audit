...
with st.sidebar:
    ...
    # НОВАЯ КНОПКА «Максимальная экономия»
    st.markdown("---")
...
for cat in monthly_cat['Категория'].unique():
    cat_data = monthly_cat[monthly_cat['Категория'] == cat].tail(3)  # Последние 3 месяца
    if len(cat_data) >= 2:
        prev = cat_data.iloc[-2]['СуммаПотерь']
        curr = cat_data.iloc[-1]['СуммаПотерь']
        growth = round((curr - prev) / prev * 100 if prev != 0 else 0)
        if abs(growth) > 20:
            trends.append({"Текст": f"Категория {cat} изменилась на {growth}% — проверить причины", "Экономия": 0, "Снижение": 0, "Цвет": "warning" if growth > 0 else "success"})
