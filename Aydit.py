...
def сгенерировать_тестовые_данные():
    ...
    категории = np.random.choice([
        'Смартфоны', 'Ноутбуки', 'Телевизоры', 'Холодильники', 'Стиральные машины',
        'Посуда', 'Столовые приборы (ложки/вилки)', 'Кухонные аксессуары',
        'Освещение', 'Мебель', 'Инструменты', 'Электрика'
    ], size=300)
...
def detect_columns(df):
    ...
    date_col = lower_columns.get(date_candidates[0], 'Дата')
    category_col = lower_columns.get(category_candidates[0], 'Категория')
    loss_col = lower_columns.get(loss_candidates[0], 'СуммаПотерь')
    store_col = lower_columns.get(store_candidates[0], 'Магазин')
...
# Фильтры + What-if + новая кнопка «Максимальная экономия»
with st.sidebar:
    st.markdown("---")
    ...
    st.markdown("---")
    if st.button("🚀 Максимальная экономия", type="primary"):
        st.session_state.reduce_a = 30
        st.session_state.reduce_peak = 25
        st.session_state.reduce_top_store = 35
        st.rerun()
...
# Детекция трендов
df_trends = df.copy()
df_trends['Месяц'] = df_trends['Дата'].dt.to_period('M').astype(str)
monthly_cat = df_trends.groupby(['Месяц', 'Категория'])['СуммаПотерь'].sum().reset_index().sort_values(['Категория', 'Месяц'])
trends = []
for cat in monthly_cat['Категория'].unique():
    cat_data = monthly_cat[monthly_cat['Категория'] == cat].tail(3)
    if len(cat_data) >= 2:
        prev = cat_data.iloc[-2]['СуммаПотерь']
        curr = cat_data.iloc[-1]['СуммаПотерь']
        growth = round((curr - prev) / prev * 100 if prev != 0 else 0)
        if abs(growth) > 20:
            trends.append({
                "Текст": f"Категория {cat} изменилась на {growth}% — проверить причины",
                "Экономия": 0,
                "Снижение": 0,
                "Цвет": "warning" if growth > 0 else "success"
            })
...
with tab5:
    ...
    for i, r in enumerate(рекомендации, 1):
        text = f"{i}. {r['Текст']} — экономия до {r['Экономия']:,.0f} ₽ при снижении на {r['Снижение']}%" if r['Экономия'] > 0 else f"{i}. {r['Текст']}"
        if r["Цвет"] == "error":
            st.error(text)
        elif r["Цвет"] == "warning":
            st.warning(text)
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        ...
        pd.DataFrame([r["Текст"] + f" — {r['Экономия']:,.0f} ₽" for r in рекомендации], columns=['Рекомендация']).to_excel(writer, sheet_name='Рекомендации', index=False)
    buffer.seek(0)
